...
const path = require('path'); //In app.js, require the Path module
const app = express();
const port = 3000;

app.use(morgan('combined'));
app.use(express.static(path.join(__dirname, 'public'))); //Note: "public" contains all assets (images, scripts.js, and styles.js)
app.set('view engine', 'ejs'); //Configure Node.js to use EJS as its views engine
...
app.get('/books/:_id', (req, res) => {
  ...
  if (params._id === '001') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[0],
      });
  } else if (params._id === '002') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[1],
      });
  } else if (params._id === '003') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[2],
      });
  } else if (params._id === '004') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[3],
      });
  } else if (params._id === '005') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[4],
      });
  } else if (params._id === '006') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[5],
      });
  } else if (params._id === '007') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[6],
      });
  } else if (params._id === '008') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[7],
      });
  } else if (params._id === '009') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[8],
      });
  } else if (params._id === '010') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[9],
      });
  } else if (params._id === '011') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[10],
      });
  } else if (params._id === '012') {
      res.render('pages/bookDetail', {
          inventoryItem: bookInventory[11],
      });
  } else {
      res.send(`This product doesn't exist. Try searching again. `);
  };
});
...
app.get('/admin-console/update-book', (req, res) => {
  res.render('pages/update');
});

// app.get('/:nameParam', (req, res) => {
//   let parameters = req.params;
//   console.log (req.params);
//   let name = parameters.nameParam;
//   res.send(`It worked ${name}. Imagine how this route could lead to the profile of ${name}, which is what was typed into the URL in place of :nameParam. Routes with parameters are awesome.`)
// })
